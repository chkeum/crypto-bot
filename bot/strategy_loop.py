...
from .config import (
  START_MODE, STRAT_ENABLE, DATA_EXCHANGE_ID, DATA_IS_TESTNET,
  STRAT_SYMBOLS, STRAT_TIMEFRAME, STRAT_QTY_USD, STRAT_LEVERAGE,
  STRAT_EMA_FAST, STRAT_EMA_SLOW, STRAT_ATR_LEN, STRAT_ATR_MA_LEN,
  STRAT_ATR_MULT, STRAT_BREAKOUT_LEN, STRAT_VOLMA_LEN, STRAT_VOL_MULT,
  CONFIRM_TF, HTF_EMA_FAST, HTF_EMA_SLOW,
  POSITION_SIZING, RISK_USD, MIN_BASE_QTY,
  BRACKET_ENABLE, BRACKET_TP_RR, BRACKET_TP_AS_MARKET, BRACKET_WORKING_TYPE,
  STRAT_POLL_SEC, STRAT_LOG_EVERY_BAR
)

def ema_series(vals: List[float], length: int) -> List[float]:
    if length <= 1: return vals[:]
    k = 2 / (length + 1); out=[]; e = vals[0]
...
def compute_atr(h: List[float], l: List[float], c: List[float], length: int) -> List[float]:
    ...
    for i in range(1,len(c)):
        tr=max(h[i]-l[i], abs(h[i]-prev), abs(l[i]-prev)); trs.append(tr); prev=c[i]
...
class StrategyLoop:
    ...
    def __init__(self, engine):
        ...
        try: self.data_ex.set_sandbox_mode(DATA_IS_TESTNET)
        except Exception: pass
        try: self.data_ex.load_markets()
        except Exception: pass
    ...
    def _get_engine_position_side(self, symbol: str) -> Optional[str]:
        if hasattr(self.engine,"positions"):  # Paper
            ...
            if not pos or abs(pos.get("amount",0.0))==0: return None
    ...
    def _split(self, ohlcv):
        ts=[r[0] for r in ohlcv]; o=[r[1] for r in ohlcv]; h=[r[2] for r in ohlcv]; l=[r[3] for r in ohlcv]; c=[r[4] for r in ohlcv]; v=[r[5] for r in ohlcv]
        return ts,o,h,l,c,v

    def _calc_ltf(self, ohlcv):
        ts,o,h,l,c,v = self._split(ohlcv)
        i=-2
        efast=ema_series(c, STRAT_EMA_FAST); eslow=ema_series(c, STRAT_EMA_SLOW)
        ...
        hh=max(h[-(STRAT_BREAKOUT_LEN+1):-1]); ll=min(l[-(STRAT_BREAKOUT_LEN+1):-1])
        brk_long=c[i]>hh; brk_short=c[i]<ll
        return {
            "ts": ts[i], "close": c[i], "atr": atr[i], "atr_ma": atr_ma, "vol_ok": vol_ok,
            "trend_ltf_long": efast[i]>eslow[i], "trend_ltf_short": efast[i]<eslow[i],
    ...
    def _calc_htf_trend(self, ohlcv_htf):
        ...
        i=-2
        efast=ema_series(c, HTF_EMA_FAST); eslow=ema_series(c, HTF_EMA_SLOW)
    ...
    async def _maybe_eval_symbol(self, symbol: str):
        ...
        if self.last_bar_ts.get(symbol) == l["ts"]:
            ...
            if hasattr(self.engine, "cancel_reduces_if_flat"):
                try: self.engine.cancel_reduces_if_flat(symbol)
                except Exception: pass
            return
        ...
        if long_ok and side_now != "long":
            ...
            logger.info(f"[ENTRY] {symbol} LONG qty={base_amt:.6f} entry~{price:.4f} SL={self.sl_price[symbol]:.4f} TP={self.tp_price[symbol]:.4f} stop={stop_distance:.4f}")

            if BRACKET_ENABLE and hasattr(self.engine, "place_bracket"):
                try:
                    self.engine.place_bracket(symbol, "long", base_amt,
                                              self.sl_price[symbol], self.tp_price[symbol],
            ...
            logger.info(f"[ENTRY] {symbol} SHORT qty={base_amt:.6f} entry~{price:.4f} SL={self.sl_price[symbol]:.4f} TP={self.tp_price[symbol]:.4f} stop={stop_distance:.4f}")

            if BRACKET_ENABLE and hasattr(self.engine, "place_bracket"):
                try:
                    self.engine.place_bracket(symbol, "short", base_amt,
                                              self.sl_price[symbol], self.tp_price[symbol],
                                              tp_as_market=BRACKET_TP_AS_MARKET,
                                              working_type=BRACKET_WORKING_TYPE)
