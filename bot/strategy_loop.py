...
class StrategyLoop:
    ...
    기능:
      ...
      - 진입 시 reduceOnly 브래킷 배치
      - '봇 태그' 저장(restore에서 봇 포지션만 복구 선택)
      - HTF/ATR/Volume 필터 토글 (env)
    """
    ...
    def __init__(self, engine):
        ...
        self.risk_use_free   = str(os.getenv("RISK_USE_FREE", "1")).lower() in ("1","true","yes","y","on")  # default FREE
        ...
        # ----- Margin check knobs -----
        self.margin_check_enable = str(os.getenv("MARGIN_CHECK_ENABLE", "1")).lower() in ("1","true","yes","y","on")
        ...
        # ----- Filter toggles (env) -----
        self.req_htf = str(os.getenv("REQUIRE_HTF_CONFIRM", "1")).lower() in ("1","true","yes","y","on")
        self.req_atr = str(os.getenv("REQUIRE_ATR_EXP", "1")).lower() in ("1","true","yes","y","on")
        self.req_vol = str(os.getenv("REQUIRE_VOL_EXP", "1")).lower() in ("1","true","yes","y","on")
        logger.info(f"[FILTER] htf={'ON' if self.req_htf else 'OFF'} atr={'ON' if self.req_atr else 'OFF'} vol={'ON' if self.req_vol else 'OFF'}")
            ...
            if hasattr(self.engine, name):
                try:
                    ...
                    if isinstance(res, dict):
                        if "equity" in res: return float(res["equity"])
            ...
            try:
                ...
                if isinstance(bal, dict):
                    ...
                    for k in ("totalWalletBalance","totalCrossWalletBalance","totalMarginBalance"):
                        val = info.get(k)
                        if val not in (None,"",0,"0"): return float(val)
            ...
            if client and hasattr(client, m):
                try:
                    ...
                    if isinstance(res, dict):
                        for k in ("totalWalletBalance","totalCrossWalletBalance","totalMarginBalance"):
                            v = res.get(k)
                            if v not in (None,"",0,"0"): return float(v)
    ...
    async def _maybe_eval_symbol(self, symbol: str):
        ...
        # --- filter-toggled conditions ---
        conds_long = [l["brk_long"], l["trend_ltf_long"]]
        conds_short = [l["brk_short"], l["trend_ltf_short"]]
        if self.req_htf:
            conds_long.append(h["trend_htf_long"])
            conds_short.append(h["trend_htf_short"])
        if self.req_atr:
            conds_long.append(l["atr"] > l["atr_ma"])
            conds_short.append(l["atr"] > l["atr_ma"])
        if self.req_vol:
            conds_long.append(l["vol_ok"])
            conds_short.append(l["vol_ok"])
        long_ok = all(conds_long)
        ...
        if long_ok and side_now != "long":
            ...
            if STRAT_LOG_EVERY_BAR:
                reasons = []
                if self.req_atr and not (l["atr"] > l["atr_ma"]): reasons.append("no_ATR_exp")
                if self.req_vol and not l["vol_ok"]: reasons.append("no_vol")
