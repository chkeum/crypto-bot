...
class PaperExchange:
    ...
    def open_market(
        self, symbol: str, side: str, base_amount: float, leverage: int = 1
    ) -> Dict[str, Any]:
        ...
        if new_amount != 0:
            pos["entry"] = (
                (pos.get("entry", 0.0) * pos["amount"] + price * signed_amount)
                / new_amount
                if pos["amount"] != 0
                else price
            )
        ...
        pos["side"] = (
            "long" if new_amount > 0 else ("short" if new_amount < 0 else None)
        )
        self.positions[symbol] = pos
        logger.info(
            f"[PAPER] Open {side} {symbol} {base_amount:.6f}@~{price:.2f} lev={leverage}"
        )
        return {
            "status": "filled",
            "symbol": symbol,
