...
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "[push+deploy] changes detected → commit"; git add -A; git commit -m "auto deploy";
else
  echo "[push+deploy] no local changes"; fi
...
LABEL_ARGS=(); REPO_NWO="$(gh repo view --json nameWithOwner --jq .nameWithOwner 2>/dev/null || true)"
IFS=',' read -ra _LBL <<<"$PR_LABELS"
for l in "${_LBL[@]}"; do l_trim="$(echo "$l" | xargs)"; [ -z "$l_trim" ] && continue
  [ -n "$REPO_NWO" ] && gh label create "$l_trim" --repo "$REPO_NWO" --color BFD4F2 --description "auto-created" >/dev/null 2>&1 || true
  LABEL_ARGS+=(--label "$l_trim"); done
...
if [ -z "${PR_NUMBER:-}" ] || [ "$PR_NUMBER" = "null" ]; then
  ...
  [ -n "$PR_URL" ] && PR_NUMBER="$(gh pr view "$PR_URL" --json number --jq '.number' 2>/dev/null || true)"; fi
