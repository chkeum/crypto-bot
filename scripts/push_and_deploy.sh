...
for l in "${_LBL[@]}"; do
  ...
  [ -z "$l_trim" ] && continue
  if [ -n "$REPO_NWO" ]; then
    gh label create "$l_trim" --repo "$REPO_NWO" --color BFD4F2 --description "auto-created" >/dev/null 2>&1 || true
  fi
...
if [ $PR_EXIT -ne 0 ]; then
  ...
  set +e
  PR_OUT="$(gh pr create --base "$BASE_BRANCH" --head "$PR_BRANCH" --title "$TITLE" --body "$BODY")"
  PR_EXIT=$?
  set -e
fi
if [ $PR_EXIT -ne 0 ]; then
  ...
  if gh pr merge "$PR_NUMBER" "$MERGE_FLAG" --auto --delete-branch >/dev/null 2>&1; then
    ...
    if [ "${FORCE_IMMEDIATE_MERGE_AFTER_AUTO}" = "1" ]; then
      ...
      if gh pr merge "$PR_NUMBER" "$MERGE_FLAG" --delete-branch >/dev/null 2>&1; then
        MERGED=1
        echo "[push+deploy] merged PR #$PR_NUMBER (immediate)"
      else
        echo "[push+deploy] immediate merge not allowed (waiting on checks or branch protection)"
      fi
    fi
  else
...
if [ "$do_deploy" = "1" ]; then
  ...
  [ -n "${PR_URL:-}" ] && echo "[push+deploy] PR URL: $PR_URL"
fi
